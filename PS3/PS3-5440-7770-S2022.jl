...
# ╔═╡ 864abff0-de42-4812-a45c-5fa3d3560baf
html"""
<p style="color:Red;font-size:16px;"> <strong>***Note:</strong> <span style="color:black;">The following equation should be the correct one and is used in the subsequent calculations.</span>***</p>
	
"""

# ╔═╡ d9939938-dbed-4953-8044-9b9c582edb7a
md"""
$$v_{i} = V_{max,i}\left[\frac{\prod_{j}\frac{S_{j}}{K_{j}}}{\prod_{j}\left(1+\frac{S_{j}}{K_{j}}\right) }\right]\qquad{i=1,2,\dots,\mathcal{R}}$$

"""

# ╔═╡ 3186bd09-5046-43bb-8f89-44ac3bbd0f37
md"""
Here are the reasons for the selections of the [Met/Km] values used for computation.

+ v1) The mouse is also mammalian and the data probably resembles the human cell well.   
+ v2) No data for substrates is available, assume reactions are at Vmax.  
+ v3) Data for humans are selected.  
+ v4) Arginine concentration in "S.cerevisiae" is 0.0218; in "E.Coli" is 0.000569; in "H. Sapiens" is 0.000255. Since Ornithine is related to Arginine synthesis, and the concentration of Arginine in E.Coli better approximates that of H. Sapiens, the [Met]/Km data for E.Coli was adopted.  
...
begin
	...
	flux_bounds_array[1,2] = 203.0*E_c1[1]
	flux_bounds_array[2,2] = 34.5*E_c1[2]
	flux_bounds_array[3,2] = 249.0*E_c1[3]
	flux_bounds_array[4,2] = 88.1*E_c1[4]
	flux_bounds_array[5,2] = 13.7*E_c1[5]
	flux_bounds_array[6,2] = 13.7*E_c1[6]
...
# ╔═╡ dc960e50-996d-411c-8ba1-eb50436d2457
begin
	ec_number_array2 = ["arginine"]
	filter_col_key2 = Symbol("Metabolite")
	df2 = filter(filter_col_key2=>x->in(x,ec_number_array2), metabolite_table)
end

# ╔═╡ c2a3ac75-61f7-481b-974c-241f0cde33ab
begin
	#adjust reaction rate by substrate concentration. 
	#If the substrate is not found in Park et. al. (2016), it is assumed that the substrate concentration does not affect reaction rate (S/K >>1).
	
	u_c2 = Array{Float64}(undef,6)
	
	#Reaction V1; Enzyme 6.3.4.5, ATP+L-Citrulline+L-Aspartate -> AMP+Diphosphate+N-(L-Arginino)succinate
	MK_v1 = Array{Float64}(undef,2);  
	#ATP
	MK_v1[1] = 11.9;
	#S_Aspartate 
	MK_v1[2] = 96.7;
	#Set up Coefficients for v1
	u_c2[1] = prod(MK_v1)/prod(1 .+MK_v1);

	#Reaction V2; Enzyme 4.3.2.1, N-(L-Arginino)succinate -> Fumarate+L-Arginine; 
	#[Met]/Km not available for substrates.
	u_c2[2] = 1

	#Reaction V3; Enzyme 3.5.3.1, L-Arginine+H2O -> L-Ornithine+Urea
	MK_v3 = Array{Float64}(undef,1)
	#Arginine; Values for the enzyme in "Homo Sapiens" is chosen since HL60 cells are human cells.
	MK_v3[1] = 0.165
	u_c2[3] = prod(MK_v3)/prod(1 .+MK_v3)
	
	#Reaction V4; Enzyme 2.1.3.3, L-Ornithine+Carbamoyl_Phosphate -> Orthophosphate+L-Citrulline
	MK_v4 = Array{Float64}(undef,1)
	#Data for E.Coli was selected.
	MK_v4[1] = 0.0119
	u_c2[4] = prod(MK_v4)/prod(1 .+MK_v4)

	#Reaction V5; Enzyme 1.14.13.39, 2*L-Arginine+4*Oxygen+3*NADPH+3*H(+) -> 2*Nitric_Oxide+2*L-Citrulline+3*NADP(+)+4*H2O
	#Assume the reaction has only one substrage Arginine
	MK_v5 = Array{Float64}(undef,1)
	MK_v5[1] = 73.1
	u_c2[5] = prod(MK_v5)/prod(1 .+MK_v5)
	
	#Reaction V6; Enzyme 1.14.13.39, 2*Nitric_Oxide+2*L-Citrulline+3*NADP(+)+4*H2O -> 2*L-Arginine+4*Oxygen+3*NADPH+3*H(+)
	u_c2[6] = 1
	...
	# what is the index of the urea flux?
	idx_b₄_c2 = findall(x->x=="b4", rna)[1]
	c_vector_c2[idx_b₄] = -1

	# compute the flux -
	result_case_2 = lib.flux(S,flux_bounds_array_c2,species_bounds_array_c2,c_vector_c2);
...
# ╔═╡ 939673d6-e314-43a3-b616-f988168edaf3
u_c2

# ╔═╡ 402fa869-9163-4362-b476-1f4adba778dc
# check:
with_terminal() do
	ef_c2 = result_case_2.exit_flag
	sf_c2 = result_case_2.status_flag

	if (ef_c2 == 0.0 && sf_c2 == 5.0)
		println("Case 1: Optimal solution found. exit flag = $(ef_c2) and status flag = $(sf_c2)")
	else
		println("Case 1: Ooops! Check your problem setup. exit flag = $(ef_c2) and status flag = $(sf_c2)")
	end
end

# ╔═╡ 18e99cf9-7172-444f-a4d9-16432c23349b
let

	# get flux values from the result -
	calculated_flux_array_c2 = result_case_2.calculated_flux_array
	...
	for i ∈ 1:ℛ
		...
		flux_table_c2[i,3] = calculated_flux_array_c2[i]*(1) # units: μmol/gDW-hr
	...
	with_terminal() do
		pretty_table(flux_table_c2; header=header_row, alignment=:l)
	end
end
